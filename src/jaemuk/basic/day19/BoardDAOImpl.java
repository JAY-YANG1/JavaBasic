...
public class BoardDAOImpl implements BoardDAO{
    ...
    private String insertSQL = "insert into board(title, userid, contents) " +
                                " values (?,?,?) ";
    private String selectSQL = " select bdno, title, userid, mid(regdate, 1, 10), " +
                               " views, thumbup from board " +
                               " order by bdno desc ";
    private String selectOneSQL = "select * from board where bdno = ?";
    private String viewSQL = " update board set views = views + 1 " +
                             " where bdno = ? ";
    private String thumbSQL = " update board set thumbup = thumbup + 1 " +
                              " where bdno = ? ";
