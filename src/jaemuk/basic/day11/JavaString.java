...
public class JavaString {
    public static void main(String[] args) {
        ...
        // 문자열 비교시 == 연산자는 값의 동등을 비교하는 것이 아니라
        // 그 값이 저장된 위치를 비교함 (참조값 비교)
        if(str1 == str2) {
            System.out.println("str1과 str2 의 저장위치가 같음");
        }

        if(str1 == str3) {
            System.out.println("str1과 str3 의 저장위치가 같음");
        }

        // 문자열 비교시 equals 메서드를 사용하면
        // 변수에 지정된 값을 비교함
        if(str1.equals(str2)) {
            System.out.println("str1과 str2 의 값이 같음");
        }

        if(str1.equals(str3)) {
            System.out.println("str1과 str3 의 값이 같음");
        }
        ...
        // 문자열에 + 연산자를 사용하면 StringBuilder 로 변환한 후
        // Append 로 문자열을 더하고 toString 으로 문자열로 반환
        // 따라서, 2개의 문자열을 합칠 때는 concat 으로
        // 그 이상은 + 연산자로 합치는 것이 나을 수 있음
        String a = "Hello";
        String b = "World";
        System.out.println(a.concat(b));

        // indexOf : 특정 문자열이 처음 등장하는 위치를 알아냄
        // 문자열이 존재하지 않을 경우 -1 을 반환함
        // 보통 문자열내에 특정문자가 포함되어 있는 지 알아낼 때 사용
        ...
        // 오른쪽을 기준으로 왼쪽 방향으로 특정 문자열의 위치 index 를 찾음
        ...
        // 문자열의 실제 바이트 수를 알아내려면 getBytes 를 사용해야함
        ...
        String words1 = (words[0].charAt(0) + "").toUpperCase();
        String words2 = (words[1].charAt(0) + "").toUpperCase();

        l = l.replace("h", words1);
        l = l.replace("j", words2);
        System.out.println(l);


        // subString : 문자열의 시작위치부터 끝위치-1 까지의
        // 문자열을 추출하는 메서드
        // subString(시작)
        // subString(시작, 끝-1)
        ...
        // format : c 언어의 printf 처럼 서식문자열을 이용해서
        ...
        // StringBuilder 나 StringBuffer 를 이용하면
        // 기존 공간에 새로운 문자열을 바로 추가할 수 있으므로
        // 메모리를 절약할 수 있음
        // 따라서, + 를 이용해서 문자열을 결합하는 것 보다는
        // StringBuilder 를 이용하는 것이 더 나음
