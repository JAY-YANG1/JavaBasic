...
public class SungJukV4DAOImpl implements SungJukV4DAO{
    ...
    private String insertSQL = "insert into sungjuk (name, kor, eng, mat, tot, mean, grd)\n" +
            "values (?,?,?,?,?,?,?)";
    private String selectSQL = "select sjno, name, kor, eng, mat, date(regdate) regdate from sungjuk\n" +
            "order by sjno";
    private String selectOneSQL = "select * from sungjuk\n" +
            "where sjno = ?;";
    private String updateSQL = " update sungjuk set name = ?, kor = ? ,eng = ?, mat = ? " +
    ...
    // Service 에서 넘겨준 성적데이터를 sungjuk 테이블에 저장
    @Override
    ...
    // 조회한 후 ArrayList 에 담아서 Service 로 넘김
    @Override
    ...
    // Service 에서 넘겨준 학생번호로 sungjuk 테이블을 조회한 후
    // 조회된 모든 결과를 SungJukVO 에 담아서 Service 로 넘김
    @Override
    public SungJukVO selectOneSungJuk(int sjno) {
        ...
        try (
            ...
            while(rs.next()) {
                sj = new SungJukVO(rs.getString(2), rs.getInt(3),
                        rs.getInt(4), rs.getInt(5));
                sj.setSjno(rs.getString(1));
