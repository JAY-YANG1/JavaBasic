...
//                Integer.parseInt(numbers2[2]));
        ...
        // => Object 타입은 바로 정수로 변환 불가라서


        // JDK 1.5 이전에서는 여러 타입을 사용하는 대부분의 클래스나
        // 메서드에서 인수나 반환값으로 Object 타입을 사용했었음
        // 이런 경우, 반환된 Object 객체를 다시 원하는 타입으로 변환해야하고
...
//                (int)numbers3[1] + (int)numbers3[2]);
        ...
        // nums2 라는 배열변수에는 String 타입의 값만 저장할 수 있음
...
//                (int)nums2.get(1)) + (int)nums2.get(2));
        // 컴파일 할때 타입검사를 해서 오류를 바로 알려줌

        System.out.println(Integer.parseInt(nums2.get(0)) +
                Integer.parseInt(nums2.get(1)) + Integer.parseInt(nums2.get(2)));
