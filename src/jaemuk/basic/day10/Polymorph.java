...
public class Polymorph {
    public static void main(String[] args) {
        ...
        // 상속관계에 있는 부모 / 자식 클래스 / 인터페이스 간에도
        // 서로 형변환이 가능
        // 이러한 형변환을 업캐스팅 / 다운캐스팅이라 함
        // 자식클래스의 타입이 부모클래스의 타입으로 바뀌는 것을
        // 업캐스팅이라 하고, 부모클래스의 타입이 자식클래스의 타입으로
        // 바뀌는 것을 다운 캐스팅이라 함
        ...
        // paint 메서드를 구현해서 draw 호출을 수월하게 바꿔봄
        // 여전히, draw 메서드 호출시 동일한 클래스 타입을 사용해야 함
        // 새로운 도형이 추가되면 paint 메서드도 추가해야 함
        paint(new Line());
        paint(new Circle());
        paint(new Triangle());
